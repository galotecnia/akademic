...
#       return val[0]
    ...
    def open(self, db_name):
        ...
        if os.path.isfile(db_name[0:-3] + 'fpt'):
        ...
        if self.header['# of Records']:
            data_size = (self.header['# of Records'] * self.header['Record Size'])
    ...
    def get_record_with_names(self, rec_no):
        ...
        if self.memo_file:
            for key in self.fields:
                ...
                c_data = record[f_name].strip()

                if f_type=='M' or f_type=='G' or f_type=='B' or f_type=='P':
                    c_data = self._reverse_endian(c_data)
                    if c_data:
                        record[f_name] = self.read_memo(c_data-1)
                else:
                    record[f_name] = str(c_data).strip()
        record['ERASED'] = record['ERASED'] == chr(0x2a)
        return record
...
if __name__=='__main__':
    #rec = readDbf('NOTAS.DBF')
    rec = readDbf('./dbfpy/libros.dat')
    f = open('/tmp/chorrilla.csv', 'w')
    f.write(','.join(rec[0].keys())+'\n')
    print rec[0].keys()
    for line in rec:
#       if line['CURSO_ESCO'] == '2007':
        f.write(str(line)+"\n")
    f.close()
