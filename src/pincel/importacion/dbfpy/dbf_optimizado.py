...
agarfu 05/25/08 007 Optimized read speed
...
class Dbf:
	...
	def openOn(self, dbfFileStream):
		...
		self.dbfData = self.dbfStream.read ()
		return(self)
...
class DbfMemoFieldDef(DbfFieldDef):
	...
	def decodeValue(self, rawval):
		...
		jjk  02/18/98'''
		if string.strip(rawval) == '': return 0
		try:
...
#		self.dbf.dbfStream.seek(pos)
#		print "recordIndex: %d" % recordIndex
#		print "HeaderSize: %d" % self.dbf.header.headerLength
#		print "rowSize: %d" % len
#		print "pos: %d" % pos
#		rawData = self.dbf.dbfStream.read(len)
#		print "'%s' [%d]" % (rawData, pos)
		rawData = self.dbf.dbfData[pos:pos + len]
#		print "'%s' [%d]" % (rawData, pos)
...
#		sys.exit (0)
	...
	def fromRawData(self, rawDataString):
		...
		jjk  02/19/98'''
		self._reset()
		self.isDeleted = (rawDataString[0] == chr(0x2A))  #del='*'  notDel=' '
